 ...
 * Ported to TypeScript for Supabase Edge Functions
 */
 ...
 */
export function calculateChunkStrategy(durationMinutes: number) {
 ...
 */
export function validateScriptQuality(script: string, research: ResearchQuality, targetWords: number) {
 ...
 */
export function validateUserAccess(userId: string, userTier: string, duration: number, model: string) {
  ...
  const errors: Array<{field: string, message: string, maxAllowed?: number, allowedModels?: string[]}> = [];
