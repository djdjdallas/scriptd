 ...
 *
 * Ported from: src/lib/script-generation/expansion-handler.js
 * Purpose: Expands short scripts by identifying gaps and generating targeted content,
...
export function analyzeContentGaps(
  ...
  const isLastChunk = chunkInfo?.isLast || false;
  ...
  if (gaps.length === 0 && wordsNeeded > 50) {
...
export async function generateExpansion(
  ...
  apiKey: string,
  model: string = Deno.env.get('BALANCED_MODEL') || 'claude-sonnet-4-5-20250929',
  chunkInfo: ChunkInfo | null = null
): Promise<string | null> {
 ...
 */
export async function expandShortScript(
  script: string,
  contentPoints: ContentPoint[] | null,
  targetWords: number,
  research: Research | null,
